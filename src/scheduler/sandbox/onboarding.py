...
This is the sandbox-side counterpart to scheduler.onboarding. Same agent,
same tools, same system prompt — but tool implementations make HTTP calls
to the control plane instead of calling Google APIs directly. No auth
tokens ever enter the sandbox.
"""
...
def _build_tools(api: ControlPlaneClient):
    """Build Agent SDK tools that call the control plane over HTTP."""
    ...
    async def read_thread(args):
        result = api.read_thread(args["thread_id"])
        return {"content": [{"type": "text", "text": json.dumps(result)}]}
    ...
    async def get_calendar_events(args):
        result = api.get_calendar_events(
            start_date=args["start_date"],
            end_date=args["end_date"],
        )
        return {"content": [{"type": "text", "text": json.dumps(result)}]}
...
async def _run_onboarding_async():
    ...
    tools, events_added = _build_tools(api)
